...
void HAL_UART_RxCpltCallback(UART_HandleTypeDef *huart) {
    if (huart->Instance == USART1) {
        ProcessUARTByte(UART_BYTE_BUFFER);
        HAL_UART_Receive_IT(&huart1, &UART_BYTE_BUFFER, 1);}
		
		if(huart->Instance == USART2){
		for(int i = 0; i<26; i++){
			if(DMA_buffer2[i] == 0xC8){
				data_type = DMA_buffer2[i+2];
				if(data_type == 0x16){
				memcpy(RxDataUart2, &DMA_buffer2[i], 26 * sizeof(RxDataUart2[0]));}}}
		unpackChannelData(RxDataUart2, RC_TBS);
		HAL_UART_Receive_DMA(&huart2, DMA_buffer2, sizeof(DMA_buffer2));}

    if (huart->Instance == USART6){
		for(int i = 0; i<26; i++){
			if(DMA_buffer6[i] == 0xC8){
				data_type = DMA_buffer6[i+2];
				if(data_type == 0x16){
				memcpy(RxDataUart6, &DMA_buffer6[i], 26 * sizeof(RxDataUart6[0]));}}}
		unpackChannelData(RxDataUart6, RC_ELRS);
		HAL_UART_Transmit_DMA(&huart6, DMA_buffer6, sizeof(DMA_buffer6));
    ...
    if (hspi == &hspi2) {
			...
			if(SPI_buffer[i] == '$' && SPI_buffer[i+1] == 'M' && SPI_buffer[i+2] == '>'){}
				...
				if(command_type == MSP_SET_RAW_RC){
					...
					RC_SPI[2] = ((RxDataSPI[5] << 8) | RxDataSPI[3]);
...
void ProcessUARTByte(uint8_t byte) {
	if (!in_packet) {
		...
		buffer[packet_pos++] = byte;
		if (packet_pos >= 26 && buffer[2] == 0x88){
				memcpy(RxDataUart1, buffer, sizeof(RxDataUart1));
...
void unpackChannelData(uint8_t *array1, uint16_t *array2){
	bitsMerged = 0;
	readValue = 0;
	readByteIndex = 3;
...
int main(void)
{
  ...
  HAL_UART_Receive_IT(&huart1, &UART_BYTE_BUFFER, sizeof(UART_BYTE_BUFFER));
	HAL_UART_Receive_DMA(&huart2, DMA_buffer2, sizeof(DMA_buffer2));
	HAL_UART_Receive_DMA(&huart6, DMA_buffer6, sizeof(DMA_buffer6));
	HAL_SPI_Receive_IT(&hspi2, SPI_buffer, sizeof(SPI_buffer));	
